...
class CommentModel extends DBSql{
    ...
    {
        return $this->SearchQuery("SELECT comment.userID, user.userName ,user.name, comment.commentText, comment.commentStatus FROM comment INNER JOIN user ON comment.userID = user.userID WHERE productID = '$productID'");
    }
